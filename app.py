...
def handle_fishing(user, seconds):
    ...
    if user.get("fishing_until", 0) > now:
        ...
        return kakao_text_response(f⚠️ 아직 낚싯대를 회수하지 않았습니다. {remain}초 정도 더 기다려야 할 것 같아요.)

    # 새 낚시 시작
    user["fishing_until"] = now + seconds

    if not user["place"]:
        user["fishing_until"] = 0
        return "⚠️ 먼저 장소를 설정해주세요. (/장소 바다 or /장소 민물)"

    bait_type = "지렁이" if user["place"] == "바다" else "떡밥"
    limit_bait = bait_type + "(제한)"
    total_bait = user["inventory"][bait_type] + user["inventory"][limit_bait]

    if total_bait <= 0:
        user["fishing_until"] = 0
        return kakao_text_response(f⚠️ {bait_type}가 부족합니다. 상점에서 구매해주세요.)

    if len(user["bag"]) >= user["max_slot"]:
        user["fishing_until"] = 0
        return kakao_text_response(f⚠️ 가방이 가득 찼습니다. ({len(user['bag'])}/{user['max_slot']}칸)\n\n" + bag_text(user))
...
def handle_buy(user, name, amount_txt):
    ...
    if user["gold"] < price:
        return kakao_text_response(f⚠️ 골드가 부족합니다. (부족한 골드: {price - user['gold']}))
    ...
    txt_limit = f" (제한골드 {used_limit} 사용)" if used_limit else ""
        return kakao_text_response(f✅ 구매 완료: {name} x{amount}{txt_limit}\n남은 Gold: 💰{user['gold']})
...
def handle_sell(user, name, amount_txt):
    ...
    user["gold"] += earn
        return kakao_text_response(f✅ 판매 완료: {name} x{amount} → 💰{earn}\n현재 Gold: 💰{user['gold']})
...
def check_in(user):
    ...
    if title == "🐟 낚린이": reward = 500
        ...
        return kakao_text_response(f⚠️ {title} 등급은 출석 보상을 받을 수 없습니다.)
    
    if reward > 0:
        user["limit_gold"] += reward
        user["last_checkin"] = today_str
        return kakao_text_response(f✅ 출석 완료! ({title}) 제한골드 {reward}이 지급되었습니다.\n(현재 제한골드: {user['limit_gold']}))
...
def set_place(user, place_txt):
    ...
    user["place"] = place
        return kakao_text_response(f🌊 낚시 장소가 [{place}]로 설정되었습니다.)
...
def handle_command(user_id, utter):
    ...
    if command == "/초기화":
        ...
        return kakao_text_response(f✅ '{target_nick}' 님의 데이터가 초기화되었습니다.)
        else:
        return kakao_text_response(f⚠️ '{target_nick}' 닉네임을 찾을 수 없습니다.)
...
def index():
    if request.method == "HEAD":
        return "", 200
    return kakao_text_response("서버 정상 작동 중 ✅")
