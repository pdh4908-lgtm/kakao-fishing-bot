...
def handle_reel(u):
    ...
    bait_text = ""
    if loc == "바다":
        if u["inventory"].get("지렁이", 0) > 0:
            add_consumable(u, "지렁이", -1)
        remain = u["inventory"].get("지렁이", 0)
        bait_text = f"지렁이({remain}마리) 남음"
    elif loc == "민물":
        if u["inventory"].get("떡밥", 0) > 0:
            add_consumable(u, "떡밥", -1)
        remain = u["inventory"].get("떡밥", 0)
        bait_text = f"떡밥({remain}개) 남음"
    ...
    msg = []
    msg.append(f"🎣 낚시 성공! {fish_obj['name']} {cm}cm ({grade}) | {bait_text}")
...
def handle_cmd(u, utt:str):
    ...
    if t in ("/", "홈", "home", "메뉴"):
        ...
        msg.append("1) /장소 [바다|민물] ← 먼저 장소를 설정하세요   (지렁이=바다용 미끼, 떡밥=민물용 미끼)")
        msg.append("2) /낚시 [1~60]s 또는 /낚시 [1~60]초 ← 해당 초 만큼 캐스팅 (예: /낚시 15s, /낚시 15초)")
        msg.append("   시간이 끝나면 자동으로 결과가 표시됩니다")
        msg.append("3) /기록 → 지금까지 낚은 물고기 기록 보기")
        msg.append("")
        msg.append("🏪 상점 이용 방법")
        ...
        msg.append("")
        msg.append(f"닉네임: {u['nickname']}")
        msg.append("[상태]")
        msg.append(f"Lv.{u['level']}  Exp: {u['exp']}/100")
        msg.append(fmt_money(u['gold'], u['restricted_gold']))
        msg.append(f"착용 낚싯대: {u['rod']}")
        msg.append("")
        msg += bag_text_lines(u)
    return "\n".join(msg)
    ...
    if t == "/상점":
        ...
        msg.append("🏪 [낚시 상점]")
        msg.append("")
        msg.append("🎣 소모품 (Consumables)")
        msg.append("- 지렁이 : 💰10  | 바다용 미끼")
        msg.append("- 떡밥   : 💰10  | 민물용 미끼")
        msg.append("- 집어제 : 💰500 | 어군을 모으는 약품")
        msg.append("- 케미라이트1등급 : 💰600 | 발광 미끼")
        msg.append("- 케미라이트2등급 : 💰350 | 발광 미끼")
        msg.append("- 케미라이트3등급 : 💰200 | 발광 미끼")
        msg.append("")
        msg.append("🪝 낚싯대 (Rods)  ※ 낚싯대는 물고기 사이즈별 확률 보정이 적용됩니다")
        msg.append("- 철제 낚싯대 : 💰1,000")
        msg.append("- 강화 낚싯대 : 💰5,000")
        msg.append("- 프로 낚싯대 : 💰20,000")
        msg.append("- 레전드 낚싯대 : 💰100,000")
        msg.append("")
        msg.append("💡 구매 방법:")
        msg.append("/구매 [아이템명] [수량]")
        msg.append("예) /구매 지렁이 10")
    return "\n".join(msg)
        ...
        if t.startswith("/낚시"):
            ...
            for ch in t:
                if ch.isdigit():
                    num = num*10 + int(ch)
            if num <= 0 or num > 60:
                return "낚시 시간은 1~60초 사이로 입력해 주세요. 예) /낚시 15s 또는 /낚시 15초"
            if bag_full(u):
                return bag_full_block_text(u)
            return handle_cast_and_reel(u, num)
...
def handle_cast_and_reel(u, seconds:int):
    if bag_full(u):
        return bag_full_block_text(u)
    msg = []
    msg.append(f"🎣 캐스팅 시작! {seconds}초 동안 기다립니다...")
    time.sleep(seconds)
    msg.append("...뭔가 걸린 것 같다!!!")
