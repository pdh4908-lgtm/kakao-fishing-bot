...
def handle_fishing(user, seconds):
    if not user["place"]:
        return [{"simpleText": {"text": "⚠️ 먼저 장소를 설정해주세요. (/장소 바다 or /장소 민물)"}}]
    ...
    if total_bait <= 0:
        return [{"simpleText": {"text": f"⚠️ {bait_type}가 부족합니다. 상점에서 구매해주세요."}}]

    if len(user["bag"]) >= user["max_slot"]:
        return [{"simpleText": {"text": f"⚠️ 가방이 가득 찼습니다. ({len(user['bag'])}/{user['max_slot']}칸)
...
" + bag_text(user)}}]
    ...
    return [
        {"simpleText": {"text": f"🎣 캐스팅...! {seconds}초 동안 기다리는 중..."}},
        {"simpleText": {"text":
f"…뭔가 걸린 것 같다!!!\\n\\n\\n\\n
