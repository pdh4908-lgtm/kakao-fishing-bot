...
def handle_master(user, parts):
    ...
    # 레벨 조정
    if field == "Lv":
        target_user["level"] = int(value)
        return f"✅ {target_nick}님의 레벨이 {value}(으)로 설정되었습니다."

    # 골드 지급
    if field == "골드":
        target_user["gold"] += int(value)
        return f"✅ {target_nick}님께 골드 {value}을(를) 지급했습니다. (현재: {target_user['gold']})"

    # 장비 지급
    if field == "장비":
        target_user.setdefault("items", {})
        target_user["items"][value] = target_user["items"].get(value, 0) + 1
        return f"✅ {target_nick}님께 '{value}' 장비를 지급했습니다."

    # 아이템 지급
    if field == "아이템":
        target_user.setdefault("items", {})
        target_user["items"][value] = target_user["items"].get(value, 0) + 1
        return f"✅ {target_nick}님께 '{value}' 아이템을 지급했습니다."
