...
def handle_fishing(user, seconds):
    ...
    # 캐스팅 연출
    print(f"Casting for {seconds} seconds...")
    time.sleep(seconds)
    ...
    fish_obj = {
        ...
        "place": user["place"], "time": datetime.now(pytz.timezone("Asia/Seoul")).strftime("%Y-%m-%d %H:%M")
    }
    user["bag"].append(fish_obj)
    user["record"].append(fish_obj) # 기록에도 추가
...
def handle_command(user_id, utter):
    ...
    if user["nickname"] is None and not command.startswith("/닉네임"):
...
h1{font-size:20px}
textarea, input[type=text]{width:100%;padding:8px;font-size:16px;box-sizing:border-box;margin-bottom:10px;}
...
<body>
  ...
  <form method="GET" action="/">
    ...
    <label>명령어 입력:</label>
    <input type="text" name="utter" placeholder="예: /닉네임 낚시왕, /장소 바다, /낚시 5초" autofocus/>
