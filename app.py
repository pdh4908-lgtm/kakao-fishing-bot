...
def handle_fishing(user, seconds):
    ...
    if user.get("fishing_until", 0) > now:
        ...
        return kakao_text_response(
            (
                f"\u2026뭔가 걸린 것 같다!!!\n\n"
                f"\U0001F3A3 낚시 성공! {fish_name} {length}cm ({size}어종) | 남은 {bait_type}: {remain_bait}개\n"
                f"가격: \U0001F4B0{gold} | 경험치 +{exp} | 장소: {user['place']}\n\n"
                f"{bag_text(user)}"
            )
        )
    ...
    if roll < 0.8:  # 성공 80%
        ...
        return kakao_text_response(
            (
                f"\u2026뭔가 걸린 것 같다!!!\n\n"
                f"아쉽게도 놓쳤습니다\u2026 \U0001F3A3 (남은 {bait_type}: {remain_bait}개)"
            )
        )
