...
def handle_fishing(user, seconds):
    ...
    return kakao_text_response(
        f"⚠️ 아직 낚싯대를 회수하지 않았습니다. {remain}초 정도 더 기다려야 할 것 같아요."
    )
    ...
    if total_bait <= 0:
        user["fishing_until"] = 0
    return kakao_text_response(
        f"⚠️ {bait_type}가 부족합니다. 상점에서 구매해주세요."
    )

    if len(user["bag"]) >= user["max_slot"]:
        user["fishing_until"] = 0
    return kakao_text_response(
        f"⚠️ 가방이 가득 찼습니다. ({len(user['bag'])}/{user['max_slot']}칸)\n\n" + bag_text(user)
    )
...
def handle_buy(user, name, amount_txt):
    ...
    if user["gold"] < price:
    return kakao_text_response(
        f"⚠️ 골드가 부족합니다. (부족한 골드: {price - user['gold']})"
    )
...
def check_in(user):
    ...
    return kakao_text_response(
        f"✅ 출석 완료! ({title}) 제한골드 {reward}이 지급되었습니다.\n(현재 제한골드: {user['limit_gold']})"
    )
...
def handle_command(user_id, utter):
    ...
    return kakao_text_response(
        f"✅ '{target_nick}' 님의 데이터가 초기화되었습니다."
    )
        else:
    return kakao_text_response(
        f"⚠️ '{target_nick}' 닉네임을 찾을 수 없습니다."
    )
