...
SHOP_PRICE = {
    "지렁이": 10, "떡밥": 10, "지렁이(제한)": 10, "떡밥(제한)": 10, "집어제": 2000,
...
def get_user(user_id):
    ...
    if user_id not in users:
        users[user_id] = {
            ...
            "inventory": {"지렁이": 0, "떡밥": 0, "지렁이(제한)": 0, "떡밥(제한)": 0},
...
def handle_fishing(user, seconds):
    ...
    if user["inventory"][bait_type] <= 0 and user["inventory"][bait_type + "(제한)"] <= 0:
        return f"⚠️ {bait_type}가 부족합니다. 상점에서 구매해주세요."

    if len(user["bag"]) >= user["max_slot"]:
        return f"⚠️ 가방이 가득 찼습니다. ({len(user['bag'])}/{user['max_slot']}칸)\n\n" + bag_text(user)

    if user["inventory"][bait_type + "(제한)"] > 0:
        user["inventory"][bait_type + "(제한)"] -= 1
    else:
    ...
    fish_info = random.choice(FISH_POOL[user["place"]][size])
    ...
    msg = [
        f"🎣 낚시 성공! {fish_name} {length}cm ({size}어종) | {bait_type}({user['inventory'][bait_type]}개 남음)",
        f"획득: 💰{gold} Gold | ✨+{exp} Exp | 장소: {user['place']}",
        "\n" + bag_text(user)
    ]
    return "\n".join(msg)
...
def handle_buy(user, name, amount_txt):
    ...
    if name in ("지렁이", "떡밥"):
        ...
        if use > 0:
            limit_amount = use // SHOP_PRICE[name]
            user["limit_gold"] -= use
            if name == "지렁이":
                user["inventory"]["지렁이(제한)"] += limit_amount
            else:
                user["inventory"]["떡밥(제한)"] += limit_amount
            amount -= limit_amount
            price -= use
            used_limit = use

    if user["gold"] < price:
        return f"⚠️ 골드가 부족합니다. (부족한 골드: {price - user['gold']})"
    ...
    return f"✅ 구매 완료: {name} x{amount + (used_limit // SHOP_PRICE[name])}{txt_limit}
남은 Gold: 💰{user['gold']}"
...
def handle_sell(user, name, amount_txt):
    ...
    if name in ("지렁이(제한)", "떡밥(제한)"):
        return "⚠️ 제한골드로 구매한 미끼는 판매할 수 없습니다."
...
def handle_command(user_id, utter):
    ...
    if command.upper() == "THRUST" or command == "/THRUST":
        now = time.time()
        last = user.get("last_thrust", 0)
        COOLDOWN = 10
        if now - last < COOLDOWN:
            return "[THRUST] skill timeout"
        user["last_thrust"] = now
        user["exp"] += 20
        user["gold"] += 100
        return "💥 THRUST 스킬 발동! +20 Exp, +100 Gold"
