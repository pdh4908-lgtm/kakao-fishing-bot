...
def handle_fishing(user, seconds):
    ...
    if user.get("fishing_until", 0) > now:
        ...
        return f"⚠️ 아직 낚싯대를 회수하지 않았습니다. {remain}초 정도 더 기다려야 할 것 같아요."
    ...
    if roll < 0.8:  # 성공 80%
        ...
        return (               f"🎣 낚시 성공! {fish_name} {length}cm ({size}어종) | 남은 {bait_type}: {remain_bait}개\n" \               f"가격: 💰{gold} | 경험치 +{exp} | 장소: {user['place']}\n\n" \               f"{bag_text(user)}"
)
    else:
        user["fishing_until"] = 0  # 종료
...
def handle_buy(user, item_name):
    if item_name not in SHOP_ITEMS:
        return f"⚠️ {item_name}은(는) 상점에 없는 아이템입니다."

    price, currency = SHOP_ITEMS[item_name]  # currency: "gold" 또는 "limit_gold"
    ...
    if currency == "gold":
        ...
        return f"✅ {item_name}을(를) 구매했습니다! 남은 골드: {user['gold']}"

    elif currency == "limit_gold":
        if user["limit_gold"] < price:
            return "⚠️ 제한골드가 부족합니다."
        user["limit_gold"] -= price
        user["inventory"][item_name] += 1
        return f"✅ {item_name}을(를) 구매했습니다! 남은 제한골드: {user['limit_gold']}"
...
def check_in(user):
    ...
    if reward > 0:
        ...
        return f"✅ 출석 완료! ({title}) 제한골드 {reward}이 지급되었습니다.\n(현재 제한골드: {user['limit_gold']})"
...
def get_required_exp(level):
    block = (level - 1) // 10       # 몇 번째 구간인지 (0:1~10, 1:11~20 …)
    base = 1000 * (4 ** block)      # 구간의 최소 경험치
    step = base // 5                # 같은 구간 내 레벨별 증가폭

    offset = (level - 1) % 10       # 구간 내 위치 (0~9)
    required = base + (offset * step)

    # 특수 규칙: 99 -> 100은 2배만 증가
    if level == 100:
        required = required * 2
