# app.py (simple version: no access gating, no requests/approvals)
import os
import time
import threading
...
def skill():
    ...
    try:
        # ── 안내: 더 이상 요청/허락, 입어가능/해제 기능은 없습니다 ─────────
        if msg in ("/입어요청", "/허락", "/입어가능", "/입어해제"):
            return KakaoResp.text("이제 별도의 활성화/허용 절차 없이 바로 사용 가능합니다. '/' 를 입력해 도움말을 확인하세요.")
