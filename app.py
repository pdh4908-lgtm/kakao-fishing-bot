...
def owned_items_summary(user):
    ...
    if z_n + z_l > 0:
        parts.append(f"지렁이(일반 {z_n} / 제한 {z_l})")
    if t_n + t_l > 0:
        parts.append(f"떡밥(일반 {t_n} / 제한 {t_l})")
    # 기타 아이템
...
def consume_bait(user, bait_type, prefer="limit_first"):
    """캐스팅 시 미끼 1개 차감. 기본은 제한골드 물량을 우선 사용."""
    k_n, k_l = bait_keys(bait_type)
...
def resolve_fishing_result(user, place, bait_type):
    ...
    msg = [
        ...
        f"| {bait_type}(제한골드 {limit_left}개 남음)",
...
def handle_buy(user, name, amount_txt):
    """구매 로직을 처리합니다. 지렁이/떡밥은 제한골드→일반골드 순서로 충전."""
    ...
    if is_bait:
        ...
        if max_by_limit > 0:
            ...
            user["inventory"][k_l] += max_by_limit
        # 남은 수량은 일반골드 결제
