...
def get_user(user_id):
    ...
    if user_id not in users:
        users[user_id] = {
            ...
            "record": [], "place": None, "last_checkin": None, "fishing_until": None
        }
...
def handle_fishing(user, seconds):
    ...
    if user.get("fishing_until"):
        remain = int(user["fishing_until"] - now)
        if remain > 0:
            return f"🎣 낚싯대를 회수하기까지 {remain}초 남았습니다."
        else:
            user["fishing_until"] = None
