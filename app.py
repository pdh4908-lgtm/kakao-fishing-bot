...
def resolve_fishing_result(user: dict, place: str, bait_type: str) -> str:
    ...
    msg = [
        "뭔가.... 걸린..것 ...같다!",
        "",
        "🎣 낚시 성공! {} {}cm ({}어종)".format(fish_name, length, size),
        "| {}(골드(거래불가) {}개 남음)".format(bait_type, limit_left),
        "| {}(일반골드 {}개 남음)".format(bait_type, normal_left),
        "획득: 💰{} Gold | ✨+{} Exp | 장소: {}".format(gold, exp, place),
        "",
        bag_text(user),
    ]
    return "\n".join(msg)
...
def handle_buy(user: dict, name: str, amount_txt: str) -> str:
    ...
    if is_bait:
        ...
        if max_by_limit > 0:
            used_limit_value = max_by_limit * unit
            user["limit_gold"] -= used_limit_value
            k_n, k_l = bait_keys(name)
            added = add_bait_with_limit(user, k_l, max_by_limit)
            if added < max_by_limit:
                return f"⚠️ {name}(거래불가)은 최대 50개까지 보유 가능합니다. {max_by_limit - added}개는 구매되지 않았습니다."
        # 남은 수량은 일반골드
        remain_cnt = amount - (used_limit_value // unit)
    ...
    txt_limit = f" (골드(거래불가) {used_limit_value} 사용)" if used_limit_value else ""
    extra = ""
    if is_bait:
        k_n, k_l = bait_keys(name)
